...
// Generating Random Numbers for Lats & Lons
var lats = [];
var lons = [];
...
while(lats.length < 75){
    var x = Math.floor(Math.random() * 75) + 1;
    if(lats.indexOf(x) === -1) lats.push(x);
}
while(lons.length < 75){
  var x = Math.floor(Math.random() * 75) + 1;
  if(lons.indexOf(x) === -1) lons.push(x);
}
var x = Math.floor(Math.random() * 75) + 1;
console.log(lats[x],lons[x]);

// Initializing First Map Screen
function initmap(){
  var options = {
      zoom: 9,
      center: {lat:lats[x],lng:lons[x]}
  } // Map Options
